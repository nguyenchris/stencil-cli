...
class StencilStart {
    ...
    async run(cliOptions) {
        ...
        await this.startBrowserSync(cliOptions, browserSyncPort, updatedStencilConfig);
    }
    ...
    async startBrowserSync(cliOptions, browserSyncPort, stencilConfig) {
        ...
        const { WATCH_LOCAL_CHECKOUT_JS, localCheckoutJsPath } = stencilConfig;
        // Watch checkoutJs assets if the flag is set, and reload the browser when they change
        if (WATCH_LOCAL_CHECKOUT_JS && localCheckoutJsPath) {
            const checkoutCssPath = path.join(localCheckoutJsPath, 'css');
            const checkoutJsPath = path.join(localCheckoutJsPath, 'js');
            this._browserSync.watch(checkoutCssPath, (event) => {
                if (event === 'change') {
                    this._browserSync.reload('*.css');
                }
            });
            this._browserSync.watch(checkoutJsPath, (event) => {
                if (event === 'change') {
